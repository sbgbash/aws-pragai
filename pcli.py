...
def download(bucket, key, filename):
    """
        ...
        ./paws-cli.py download --bucket gdelt-open-data --key events/\
        1979.csv --filename 1979.csv
    """

    if not bucket and not key and not filename:
        click.echo("--bucket and --key and --filename required")
        sys.exit(1)
    click.echo("Downloading s3 file with:\
